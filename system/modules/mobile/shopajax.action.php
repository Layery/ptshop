...
/* 		if(ROUTE_A!='userphotoup' and ROUTE_A!='singphotoup'){
	...
	public function getUserBuyList(){
        ...
        //根据订单号分组查询，得到订单号
        //$same_order = $this->db->GetList("select * from `@#_member_go_record` WHERE `code` in(select concat_ws(',',`code`) from `@#_member_go_record` WHERE `uid`='$member[uid]' AND `code_tmp` = 1 GROUP BY `code`)",array('key'=>'code'));
        $order_code = $this->db->GetList("select `code` from `@#_member_go_record` WHERE `uid`='$member[uid]' AND `code_tmp` GROUP BY `code`");
        $same_order = array();
        foreach ($order_code as $v){
            $same_order[$v['code']] = $this->db->GetList("select * from `@#_member_go_record` WHERE `code` = '$v[code]'");
        }
        //p($same_order);exit;
        switch ($state){
            case -1:
                //查询所有的订单
                $shoplist = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]'",array('key'=>'code'));
                $shoplistall['listItems'] = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]' GROUP BY `code` order by `time` DESC limit $FIdx,$EIdx");
                break;
            case 1:
                //查询待付款的订单
                $shoplist = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]' AND `status`='未付款,未发货,未完成'",array('key'=>'code'));
                foreach ($shoplist as $item) {
                    $startTime = $item['time'];
                    $startDate = date('Y-m-d H:i:s',$startTime);
                    $endTime = strtotime("$startDate +2 days");
                    //判断用户下所以未付款的订单的有效期
                    if($endTime <= time()){
                        $this->db->Query("update `@#_member_go_record` set `status`='未付款,未发货,已关闭' WHERE `code`='$item[code]' AND `uid`='$member[uid]'");
                    }else{
                        $shoplistall['listItems'] = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]' AND `status`='未付款,未发货,未完成' GROUP BY `code` order by `time` DESC limit $FIdx,$EIdx");
                    }
                    //echo $startDate.'<br>'.$validTime.'<br>';
                }
                $shoplistall['listItems'] = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]' AND `status`='未付款,未发货,未完成' GROUP BY `code` order by `time` DESC limit $FIdx,$EIdx");
                break;
            case 2:
                //查询待发货的订单
                $shoplist = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]' AND `status`='已付款,未发货,未完成'",array('key'=>'code'));
                $shoplistall['listItems'] = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]' AND `status`='已付款,未发货,未完成' GROUP BY `code` order by `time` DESC limit $FIdx,$EIdx");
                break;
            case 3:
                //查询待收货的订单
                $shoplist = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]' AND `status`='已付款,已发货,未完成'",array('key'=>'code'));
                foreach ($shoplist as $item) {
                    $startTime = $item['time'];
                    $startDate = date('Y-m-d H:i:s', $startTime);
                    $endTime = strtotime("$startDate +15 days");
                    //判断用户下所以未付款的订单的有效期
                    if ($endTime <= time()) {
                        $this->db->Query("update `@#_member_go_record` set `status`='已付款,已发货,已完成' WHERE `code`='$item[code]' AND `uid`='$member[uid]'");
                    } else {
                        $shoplistall['listItems'] = $this->db->GetList("select * from `@#_member_go_record` where uid='$member[uid]' AND `status`='已付款,已发货,未完成' GROUP BY `code` order by `time` DESC limit $FIdx,$EIdx");
                    }
                }
                break;
        }
        ...
        if(!empty($shoplistall['listItems'])){
            ...
            foreach($shoplistall['listItems'] as $key=>$val){
                $shoplistall['listItems'][$key]['pro_info'] = json_encode(unserialize($val['pro_info']));
                switch ($val['status']){
                    case '未付款,未发货,未完成':
                    ...
                    case '已付款,已发货,未完成':
                        ...
                        break;
                    case '已付款,已发货,已完成':
                        $shoplistall['listItems'][$key]['codeState']=4;
                        break;
                }
            }
            //p($shoplistall);exit;
        ...
        foreach ($same_order as $key => $val){
            foreach ($shoplistall['listItems'] as $k => $v){
                if($key == $v['code']){
                    foreach ($val as $ko => $vo){
                        $vo['pro_info'] = json_encode(unserialize($vo['pro_info']));
                        switch ($vo['status']){
                            case '未付款,未发货,未完成':
                                $vo['codeState']=1;
                                break;
    ...
    public function delOrder(){
        //获取订单号
        $orderCode = safe_replace($_POST['ordercode']);
        //查询订单
        if(empty($orderCode)){
            $orderInfo['code'] = 1;
            $orderInfo['msg'] = "订单不存在";
        }
        $order = $this->db->GetList("select `code` from `@#_member_go_record` WHERE `code` = '$orderCode' AND `status`='未付款,未发货,未完成'");
        if(count($order)>=1){
            //有数据，进行删除
            $res = $this->db->Query("delete from `@#_member_go_record` WHERE `code`='$orderCode'");
            if($res){
                $orderInfo['code'] = 0;
                $orderInfo['msg'] = "订单取消，欢迎再次选购！";
            }
        }else{
            $orderInfo['code'] = 1;
            $orderInfo['msg'] = "订单不存在";
    ...
    public function confirmAccept(){
        ...
        $order = $this->db->GetList("select `code` from `@#_member_go_record` WHERE `code` = '$orderCode' AND `status`='已付款,已发货,未完成'");
        if(count($order)>=1){
            //有数据，进行删除
            $res = $this->db->Query("update `@#_member_go_record` set `status`='已付款,已发货,已完成' WHERE `code`='$orderCode'");
            if($res){
                $orderInfo['code'] = 0;
                $orderInfo['msg'] = "确认收货，欢迎再次选购！";
            }
        }else{
            $orderInfo['code'] = 1;
            $orderInfo['msg'] = "订单不存在";
