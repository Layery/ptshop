...
<form name="myform" action="" method="post" enctype="multipart/form-data">
	<table width="100%" cellspacing="0">
		<tr>
			<td width="120" align="right">轮播图名称:</td>
			<td>
				<input type="text" name="title" value="<?php echo $wap['title']?>" class="input-text wid300" />
				<span><font color="#0c0">※ </font>推荐填写，用来列表查看；可以为空</span>
			</td>
		</tr>
		<tr>
			<td width="120" align="right">所属商城:</td>
		  	<td>
				<select name="category" class="wid120">
					<option value="0" <?php if($wap['where_is'] == 0)echo 'selected';?>>≡ 活动商城 ≡</option>
					<option value="1" <?php if($wap['where_is'] == 1)echo 'selected';?>>≡ 装备商城 ≡</option>
					<option value="2" <?php if($wap['where_is'] == 2)echo 'selected';?>>≡ 一元拼商城 ≡</option>
				</select>
				<span><font color="#0c0">※ </font>选择对应的商城作为轮播图</span>
		  	</td>
	  	</tr>
		<tr>
			<td width="120" align="right">图片链接:</td>
			<td>
				<input type="text" name="link" value="<?php echo $wap['link']?>" class="input-text wid300"/>
				<span><font color="#0c0">※ </font>链接可以到活动管理或装备管理进行复制</span>
			</td>
		</tr>
        <!--<tr>
			<td align="right" width="120">背景颜色：</td>
			<td>
				<input  type="text" id="title_style_color" name="title2" onKeyUp="return gbcount(this,100,'texttitle2');"  class="input-text wid300">
		...
		<tr>
            ...
            <td>
				<img height="50px" id="show-img" src="<?php echo G_UPLOAD_PATH.'/'.$wap['img']?>"/>
				<input type="text" name="image" id="imagetext" value="<?php echo $wap['img']?>" id="imagetext" class="input-text wid300">
				<input type="button" class="button" onClick="GetUploadify('<?php echo WEB_PATH; ?>','uploadify','缩略图上传','image','banner',1,500000,'imagetext','call_back')"
             value="上传图片"/>
            </td>
        </tr>
		<tr>
