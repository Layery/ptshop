...
class member extends admin {
	...
	public function insert(){
		...
		if(isset($_POST['submit'])){
			...
			$sql="INSERT INTO `@#_member` (username,img,email,mobile,password,money,score,emailcode,mobilecode,qianming,time) value ('$username','$img','$email','$mobile','$password','$money','$score','$emailcode','$mobilecode','$qianming','$time')";
	...
	public function modify(){
		...
		if(isset($_POST['submit'])){
			...
			$sql="UPDATE `@#_member` SET `username`='$username',`email`='$email',`mobile`='$mobile',`password`='$password',`money`='$money',`score`='$score',`emailcode`='$emailcode',`mobilecode`='$mobilecode',`img`='$img',`qianming`='$qianming' WHERE `uid`='$uid'";
	...
	public function del(){
		...
		$q1 = true;//$this->db->Query("insert into `@#_member_del` select * from `@#_member` where uid='$uid'");

		$q2 = $this->db->Query("delete from `@#_member` where uid='$uid'");		

		$q3 = $this->db->Query("delete from `@#_member_band` where `b_uid`='$uid'");
		/*echo "insert into `@#_member_del` select * from `@#_member` where uid='$uid'";
        var_dump($q1);
		var_dump($q2);
		var_dump($q3); 
	...
	//会员组
		...
		/*public function member_fufen(){
			...
			if(isset($_POST['submit'])){			
                ...
                $sanji_yongjin = trim($_POST['sanji_yongjin']);
...
//            echo $this->tpl(ROUTE_M,'member_insertfufen');
