...
<form method="post" action="">
	<table width="100%"  cellspacing="0" cellpadding="0">
        ...
        <tr>
			...
			<td>
            <input  type="text" id="attr_name"  name="attr_name" class="input-text bg" value="<?php echo $attribute['attr_name']?>">
            </td>
		</tr>
		<tr style="background-color:#FFC">
			<td style="width:120px"></td>
			<td>
				<b>提示：</b> <font color="red">属性类型是前台页面的显示效果，唯一属性表示商品的属性描述，用户只能才看，单选属性表示商品的属性可以选择</font><br />
			</td>
		</tr>
		<tr>
            <td align="right" style="width:120px">属性类型：</td>
            <td>
            <?php if($attribute['attr_type']){?>
                <label><input type="radio" name="attr_type" value="0"  /> 唯一属性&nbsp;&nbsp;</label>
                <label><input type="radio" name="attr_type" value="1" checked="checked"/> 单选属性&nbsp;&nbsp;</label>
            <?php }else{?>
		...
		<tr>
            ...
            <td>
            <?php if($attribute['attr_input_type']){?>
                <label><input type="radio" name="attr_input_type" value="0"  /> 手工录入&nbsp;&nbsp;</label>
                <label><input type="radio" name="attr_input_type" value="1" checked="checked"/> 列表选择&nbsp;&nbsp;</label>
            <?php }else{?>
                <label><input type="radio" name="attr_input_type" value="0" checked="ckecked" /> 手工录入&nbsp;&nbsp;</label>
                <label><input type="radio" name="attr_input_type" value="1" checked="checked"/> 列表选择&nbsp;&nbsp;</label>
            <?php }?>
            </td>
		</tr>
        ...
        <tr>
			...
			<td>
    			<textarea name="attr_value" class="wid400" onKeyUp="gbcount(this,250,'textdescription');" style="height:60px">
					<?php echo trim($attribute['attr_value'])?>
    			</textarea><br><span>还能输入<b id="textdescription">250</b>个字符</span>
            </td>
		</tr> 
        <tr height="60px">
			<td align="right" style="width:120px"></td>
			<td>
			    <input type="hidden" name="attr_id" value="<?php echo $attribute['id']?>"/>
			    <input type="submit" name="dosubmit" class="button" value="修改属性" />&nbsp;&nbsp;
...
<script type="text/javascript">
	...
	$(function(){
        ...
        //获取属性录入方式对象
        if($('input[name=attr_input_type]:checked').val() == 0){
        ...
        //去除属性值的前后空格
        var attr_value = $.trim($('textarea').val());
        $('textarea').val(attr_value);
    });
</script>
</body>
</html> 