...
class wxloginsy extends SystemAction {
	...
	public function __construct(){	
		...
		//注:修改微信重复登陆
		$user_db=System::load_app_class('base','member');
		$user=$user_db->get_user_info();
		if($user){
            $homePage = json_decode(stripslashes(_getcookie('homePage')),true);
            if($homePage == 1){
                header("Location:".WEB_PATH."/mobile/activity/activityhome");
            }else{
                header("Location:".WEB_PATH."/mobile/mobile/");exit;
            }
		}
	}
	...
	public function init(){
        session_start();
        $_SESSION['http_referer'] = $_SERVER['HTTP_REFERER'].'/0';
        $url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid='.$this->conf['weixin']['id'].'&redirect_uri='.WEB_PATH.'/api/wxloginsy/callback&response_type=code&scope=snsapi_userinfo&state=wechat123&connect_redirect=1#wechat_redirect';
		header("location:$url");
	}
	//wexin回调
	...
	private function qq_add_member(){
		...
		if($go_y_user)$go_user_info['nickname'];
