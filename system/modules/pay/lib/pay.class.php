...
class pay {
	...
	private function go_record(){
        ...
        if (!empty ( $shoparr )) {
            foreach ( $shoparr as $key => $val ) {//购物车商品的基本信息
                $is_group = isset($Mcartlist[$key]['is_group'])?$Mcartlist[$key]['is_group']:0; //团购参数
                if($is_group){
                    $is_head = isset($Mcartlist[$key]['is_head'])?$Mcartlist[$key]['is_head']:1;
                    $group_code = isset($Mcartlist[$key]['groupOrderCode'])?$Mcartlist[$key]['groupOrderCode']:'';
                }
                if(!isset($Mcartlist[$key])){
                    foreach ($item_price_info as $k => $v){//购物车对应商品的具体不同规格的货品信息
                        if($key == $v['goods_id']){ //同件商品，不同属性
                            if ($val ['surplus'] != 0 && $v['inventory'] != 0) { //商品的基本信息的剩余量和货品的剩余量
                                $shoplist [$k] = $v;
                                $shoplist [$k] = $val['group_price'];
                                $shoplist [$k]['title'] = $val['title'];
                                $shoplist [$k]['goods_img'] = $Mcartlist[$k]['goods_img'];
                                $shoplist [$k]['flag'] = intval($Mcartlist[$k]['flag']);
                                $shoplist [$k]['is_group'] = $is_group;
                                $shoplist [$k]['is_head'] = $is_head;
                                $shoplist [$k]['group_code'] = $group_code;
                                if($is_group){
                    ...
                    $shoplist [$key]['group_price'] = $val['group_price'];
	...
	//生成订单
	public function set_dingdan($pay_type='',$dingdanzhui='',$group_pre=''){
        $uid=$this->members['uid'];
		...
		$groupcode = '';
		if (!empty($group_pre)) {
			$this->groupcode = $groupcode= pay_get_dingdan_code($group_pre);
		}
		// $this->groupcode = $groupcode = pay_get_dingdan_groupcode($dingdangroupcode);
		// $this->scookies_arr['groupcode'] = $this->groupcode;
        ...
        foreach($this->shoplist as $key=>$shop){
            ...
            if($is_head == 0){
                $this->groupcode = $groupcode = $shop['group_code'];
            }
            if($shop['goods_count']){
                $insert_html.="('$dingdancode','$groupcode',$is_head,'$dingdancode_tmp','$uid','$username','$uphoto','$shop[goods_id]','$shop[title]','$pro_id','$pro_info','$shop[goods_count]','$money','$pay_type','$ip','$status','$time','$addrId','$remark'),";
            }
        }
        $sql="INSERT INTO `@#_member_go_record` (`code`,`group_code`,`is_head`,`code_tmp`,`uid`,`username`,`uphoto`,`shopid`,`shopname`,`pro_id`,`pro_info`,`gonumber`,`moneycount`,`pay_type`,`ip`,`status`,`time`,`address`,`remark`) VALUES ";
	...
	**/
     ...
     * 初始化活动支付信息
     */
    public function act_init($uid=null,$param=array(),$signInfo=array()){
        if(empty($uid) || empty($param) || empty($signInfo)){
            return false;
        }
        $this->db=System::load_sys_class('model');
        $this->db->Autocommit_start();
        ...
        $this->MoneyCount = $payment;
        $this->scookie = $signInfo;  //把报名信息存入cookie中
        $this->scookie['uid'] = $uid;
        $this->scookie['param'] = $param;
        $this->param = $param;
        return true;
    }
    /**
     * 活动支付处理
     */
    public function act_pay_action($pay_type='微信支付',$pre = ''){
        $time = time();
        $member = $this->members;
        $uid = $member['uid'];
        $param = $this->param;
        //生成订单
        $insert_order = $this->set_act_order($pay_type,'D',$uid,$param);

        //生成订单后更新报名表的支付状态
        $actId = $this->scookie['actId'];
        $sId = $this->scookie['sId'];
        $actOrderId = $this->scookie['actOrderId'];
        $shareUid = $this->scookie['shareUid'];
        $num = 1;
        //判断活动是否免费以及可报名
        $activity = $this->db->GetOne("select * from `@#_activity` WHERE `act_id`=$actId AND `act_active`=1 AND `act_start_time`>=$time");

        //更新活动表的报名人数
        $query_act = $this->db->Query("UPDATE `@#_activity` SET `act_num_signed` = `act_num_signed` + $num WHERE `act_id`= $actId");
        $query_sign = $this->db->Query("UPDATE `@#_act_sign` SET `s_status` = '已支付' WHERE `s_id` = $sId AND `s_uid` = $uid");
        //更新活动订单表的支付状态
        $query_order = $this->db->Query("UPDATE `@#_act_order` SET `o_status` = '已支付' WHERE `o_sid` = $sId AND `o_uid` = $uid AND `o_id` = $actOrderId");
        //更改积分和用户余额
        $integral = intval($activity['give_integral']);
        $user_integral = $member['score']+$integral-$param['integral'];
        $acive_balance = $member['money']-$param['balance'];
        $query_member = $this->db->Query("UPDATE `@#_member` SET `score`= '$user_integral',`money`=$acive_balance WHERE `uid`='$uid'");
        //分享消费活动积分
        if($shareUid != $uid && !empty($shareUid)){
            //查询分享的用户是否存在
            $shareUser = $this->db->GetOne("select * from `@#_member` WHERE `uid`=$shareUid");
            if($shareUser){
                $score = (int)$shareUser['score']+(int)$activity['share_integral'];
                $query_share = $this->db->Query("UPDATE `@#_member` SET `score`= $score WHERE `uid`=$shareUid");
                if($shareUser['wxid']){ //发送模板消息
                    ...
                    //查询模板消息id
                    $template_sc = $this->db->GetOne("SELECT * FROM `@#_wxch_cfg` WHERE `cfg_name` = 'template_sc'");
                    $template_id = $template_sc['cfg_value'];
                    $url = WEB_PATH.'/mobile/home';
                    //2.组装数组
                    $template = array(
                        'touser'=>$shareUser['wxid'],
                        'template_id'=>$template_id,
                        'url'=>$url,
                        'topcolor'=>'#7B68EE',
                        'data'=>array(
                            'first'=>array('value'=>urlencode('恭喜，您的分享消费成功，具体内容如下：'),'color'=>'#2B2A2A'),
                            'keyword1'=>array('value'=>urlencode(date('Y-m-d H:i')),'color'=>'#2B2A2A'),
                            'keyword2'=>array('value'=>urlencode($activity['share_integral']),'color'=>'#2B2A2A'),
                            'keyword3'=>array('value'=>urlencode('分享返积分'),'color'=>'#2B2A2A'),
                            'keyword4'=>array('value'=>urlencode($score),'color'=>'#2B2A2A'),
                            'remark'=>array('value'=>urlencode('感谢您的分享，点击查看详情'),'color'=>'#2B2A2A'),
                        )
                    );
        ...
        if(isset($param['record_id'])){
            $query_record = $this->db->Query("UPDATE `@#_member_activity_record` SET `ordercode` = '$orderCode' where `id` = {$param['record_id']}");
        }
        if($insert_order && $query_act && $query_sign && $query_order && $query_member && $query_record){
            $this->db->Autocommit_commit();
            _setcookie('act_id',json_encode($actId),''); //用来判断活动支付成功后跳转页面
            return true;
        }else{
            $this->db->Autocommit_rollback();
            return false;
    ...
    public function set_act_order($pay_type='',$pre='',$uid=null,$param=array()){
        ...
        $user_name = $this->scookie['userName'];
        $user_tel = $this->scookie['userTel'];
        $user_id = $this->scookie['userId'];
        //p($signInfo);exit;
        //数据插入报名表
	...
	public function act_pay($data=null){
        ...
        $query = $this->db->Query("INSERT INTO `@#_member_activity_record` (`uid`, `code`, `money`, `pay_type`, `status`,`time`,`scookies`) VALUES ('$uid', '$dingdancode', '$money', '$pay_type','未支付', '$time','$scookies')");
        if($query){
            $this->db->Autocommit_commit();
        }else{
            $this->db->Autocommit_rollback();
