...
class wechat extends admin {
	...
	public function cfg(){	
		...
		if(isset($_POST['dosubmit']) && $_POST['dosubmit'] != 'del'){
			if(empty($_POST['murl']) || empty($_POST['baseurl']) || empty($_POST['imgpath']) || empty($_POST['userpwd']) || empty($_POST['template_fh'])){
			...
			$template_act = htmlspecialchars(trim($_POST['template_act']));
			$openid = htmlspecialchars(trim($_POST['openid']));
			$template_re = htmlspecialchars(trim($_POST['template_re']));
			$template_tk = htmlspecialchars(trim($_POST['template_tk']));
			$template_sc = htmlspecialchars(trim($_POST['template_sc']));
			...
			$this->db->Query("UPDATE `@#_wxch_cfg` SET `cfg_value`='$template_act' WHERE (`cfg_name`= 'template_act')");
			$this->db->Query("UPDATE `@#_wxch_cfg` SET `cfg_value`='$openid' WHERE (`cfg_name`= 'openid')");
			$this->db->Query("UPDATE `@#_wxch_cfg` SET `cfg_value`='$template_re' WHERE (`cfg_name`= 'template_re')");
			$this->db->Query("UPDATE `@#_wxch_cfg` SET `cfg_value`='$template_tk' WHERE (`cfg_name`= 'template_tk')");
			$this->db->Query("UPDATE `@#_wxch_cfg` SET `cfg_value`='$template_sc' WHERE (`cfg_name`= 'template_sc')");
			_message("设置更新成功");
