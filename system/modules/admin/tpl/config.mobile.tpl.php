...
<table width="100%" class="meg_table">
     <tr>
    	<td width="100"><b>阿里大于（推荐）</b></td> 
   		<td key="1" class="mobile_on_off"></td>
     </tr>      
    <tr>
    	<td width="100">App Key：</td> 
   		<td><input type="text" name="mid" class="input-text wid150"  value="<?php echo $mobiles['cfg_mobile_1']['mid']; ?>"></td>
    </tr>
    <tr>
    	<td width="100">APP Secret：</td> 
   		<td><input type="password" name="mpass" class="input-text wid150"  value="******">
    </tr>
	<tr>
    	<td width="100">短信签名：</td>
   		<td><input type="text" name="mqianming" class="input-text wid150"  value="<?php echo $mobiles['cfg_mobile_1']['mqianming']; ?>">
		不可为空! 格式为: <font color="red">你的签名（无须添加【】、()、[]符号）必须与阿里大于官网签名配置一致</font>
		</td>
    </tr>
    <tr>
    	<td width="100">短信模板id：</td>
   		<td><input type="text" name="templateid" class="input-text wid150"  value="<?php echo $mobiles['cfg_mobile_1']['templateid']; ?>">
		 请到阿里大于官网配置短信模板，获取短信模板id，如：<font color="red">SMS_12971515</font>
		</td>
    </tr>
     <tr>
...
.p_win_title a{float:right;padding:0px 10px;color:#D80000}
...
.p_win_but a{ padding:8px 15px; background:#D80000; color:#fff;border:1px solid #f50; margin:0px 15px;font-family: "Helvetica Neue",\5FAE\8F6F\96C5\9ED1,Tohoma; font-size:15px; }
.p_win_but a:hover{ background:#f50}
.p_win_text a{ font-size:13px; color:#D80000}
