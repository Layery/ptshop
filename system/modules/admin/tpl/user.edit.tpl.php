...
<table width="100%" class="lr10">
    ...
    <td>所属角色</td>
        <td>
        <select name="gl">
        <option value="0" <?php echo$info['gl']==0?' selected = "selected"':''?>>超级管理员</option>
		<option value="1" <?php echo$info['gl']==1?' selected = "selected"':''?>>普通管理员</option>
        </select>
        </td>
    </tr>
...
$data = include G_APP_PATH.'1.php';
$qx = $info['qx'];
foreach ($data as $k=>$v ) {
    echo ' <tr><td>'.$k.'</td><td>';
	foreach ($v as $a=>$b ) {
		$mk = unserialize($qx);
		$xz = $mk[$b]=='1'?'checked="checked"':'';
	    echo '<input name="qx['.$b.']" type="checkbox" value="1" '.$xz.'/>'.$a.'&nbsp;&nbsp;';
	}
