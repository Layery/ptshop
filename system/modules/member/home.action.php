...
//福分、经验添加			
	...
	//商品佣金二维码
			public function sperweima(){
				$shoplist=$this->segment(4);
				
				$mysql_model=System::load_sys_class('model');
        $member=$this->userinfo;
         $uid=$member['uid'];
		 $uid2=$member['yaoqing'];
		 $uid3=$member['yaoqing2'];
		 $newyao=$uid;
		if (!empty($uid2)){
$newyao=$uid.'/'.$uid2;
}
...
$newyao=$uid.'/'.$uid2.'/'.$uid3;
				...
				include_once((dirname(dirname(dirname(dirname(__FILE__)))).'/system/phpqrcode/phpqrcode.php'));
$ss=QRcode::png(WEB_PATH."/mobile/mobile/item/".$shoplist."/".$newyao);
	...
	public function invitefriends(){
        ...
        $involvedtotal2=0;  // 二级人数
        $involvedtotal3=0;  // 三级人数

        //查询邀请好友信息
		$uids = 'A|'.$member['uid'];
        $invifriends=$mysql_model->GetList("select * from `@#_member` where `yaoqing1` like '%$uids%' ORDER BY `time` DESC");
		if($y>0 && $y<=3){
		    switch ($y) {
		        case '1':
		            $uids = 'A|'.$member['uid'];
		...
		$uids2 = 'B|'.$member['uid'];
        $uids3 = 'C|'.$member['uid'];
		$invifriends2=$mysql_model->GetList("select * from `@#_member` where `yaoqing1` like '%$uids2%' ORDER BY `time` DESC");
		$invifriends3=$mysql_model->GetList("select * from `@#_member` where `yaoqing1` like '%$uids3%' ORDER BY `time` DESC");// name like '%$name%'";
        $involvedtotal=count($invifriends);
		$involvedtotal2=count($invifriends2);
		$involvedtotal3=count($invifriends3);
        $invifriends4=$mysql_model->GetList("select * from `@#_member` where `yaoqing1` like '%$uids%' ORDER BY `time` DESC");
//        echo "select * from `@#_member` where `yaoqing1` like '%$uids%' ORDER BY `time` DESC";
        //var_dump($invifriends);
        
        for($i=0;$i<count($invifriends4);$i++){
//			var_dump($invifriends4);
            $sqluid=$invifriends4[$i]['uid'];
			$yaoqing1 = explode('@',$invifriends4[$i]['yaoqing1']);
            $sqname=get_user_name($invifriends4[$i]);
            $invifriends4[$i]['sqlname']=	 $sqname;
            if(in_array('A|'.$member['uid'],$yaoqing1)){
			    $jb[$sqluid] = '一级';
			}

            if(in_array('B|'.$member['uid'],$yaoqing1)){
			    $jb[$sqluid] = '二级';
			}
            if(in_array('C|'.$member['uid'],$yaoqing1)){
			    $jb[$sqluid] = '三级';
			}
            //查询邀请好友的消费明细
	...
	public function commissions(){
		...
		$zhichutotal=0;
		$uids = $member['uid'];
		$invifriends=$mysql_model->GetList("select * from `@#_member` where `yaoqing1` like '%$uids%' ORDER BY `time` DESC");//`yaoqing`='$member[uid]'
		  ...
		  for($i=0;$i<count($invifriends);$i++){
			...
			if($recodes[$sqluid][0]['jb']=='1'){
			    $vv[$sqluid] = '一级';
			}
			if($recodes[$sqluid][0]['jb']=='2'){
			    $vv[$sqluid] = '二级';
			}
			if($recodes[$sqluid][0]['jb']=='3'){
			    $vv[$sqluid] = '三级';
			}		   
...
//`yaoqing`='$member[uid]'
	...
	public function qqclock(){
		...
		//2015-8-10 添加微信绑定
	public function weixinclock(){
		$member=$this->userinfo;
		include templates("member","weixinclock");
	}
	# 我的修改 福分签到
