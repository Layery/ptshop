...
<form method="post" action="">
	...
	<table width="100%" id="general-table" cellspacing="0" cellpadding="0">
        <tr>
			...
			<td colspan="2">
            <input  type="text" id="title"  name="title" onKeyUp="return gbcount(this,100,'texttitle');"  class="input-text wid400 bg" value="<?php echo $act_cookie['title']?>">
		...
		<tr>
			...
			<td colspan="2">
                <?php if(isset($act_cookie['active'])){?>
                    <?php if($act_cookie['active']){?>
		...
		<tr>
			...
			<td colspan="2">
				<label for="recommend"><input name="recommend" id="recommend" value="1" type="checkbox" <?php if($act_cookie['recommend']){echo 'checked';}else{echo '';}?> />&nbsp;推荐&nbsp;</label>&nbsp;
				<label for="best"><input name="best" id="best" value="1" type="checkbox" <?php if($act_cookie['best']){echo 'checked';}else{echo '';}?> />&nbsp;精品</label>&nbsp;
				<label for="sale"><input name="sale" id="sale" value="1" type="checkbox" <?php if($act_cookie['sale']){echo 'checked';}else{echo '';}?> />&nbsp;特价</label>&nbsp;
		...
		<tr>
			...
			<td colspan="2">
				<input name="startTime" type="text" id="posttime1" class="input-text posttime"  readonly="readonly" value="<?php if(!empty($act_cookie['startTime'])){echo date('Y-m-d H:i:s', $act_cookie['startTime']);}?>" /> -
				<input name="endTime" type="text" id="posttime2" class="input-text posttime"  readonly="readonly" value="<?php if(!empty($act_cookie['endTime'])){echo date('Y-m-d H:i:s', $act_cookie['endTime']);}?>" />
				<script type="text/javascript">
					date = new Date();
					Calendar.setup({
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" name="address" id="address" class="input-text wid300" value="<?php echo $act_cookie['address']?>" />
			</td>
		</tr>
		<tr>
			<td align="right" style="width:120px">活动坐标（经纬度）：</td>
			<td colspan="2">
				<input type="text" name="latlng"  id="latlng" class="input-text" value="<?php echo $act_cookie['latlng']?>" />
		...
		<tr>
			...
			<td colspan="2">
				<div class="act-charge">
					活动费用
					<input type="text" name="charge" id="charge" class="input-text" style="width: 120px;" placeholder="免费请填0" value="<?php echo $act_cookie['charge']?>" />&nbsp;&nbsp;
					名额限制
					<input type="text" name="number" id="num-of-people" class="input-text" style="width: 120px;" placeholder="名额(不填默认无限制)" value="<?php echo $act_cookie['numOfPeople']?>" />
		...
		<tr>
			...
			<td width="500">
                <?php if(!empty($act_cookie['num'])){?>
                    <?php foreach ($act_cookie['num'] as $key => $val){?>
                        <div class="price-step">
                            人数达到
                            <input type="text" name="num[]" value="<?php echo $val?>" onKeyUp="value=value.replace(/\D/g,'')" id="num_step" class="input-text" placeholder="请输入阶梯人数"/>&nbsp;&nbsp;
                            享受价格
                            <input type="text" name="price[]" value="<?php echo $act_cookie['price'][$key]?>" id="enjoy_price" class="input-text" style="width: 120px;"" />
                            <?php if($key == 0){?>
                ...
                <?php }else{?>
                    <div class="price-step">
                        人数达到
                        <input type="text" name="num[]" onKeyUp="value=value.replace(/\D/g,'')" id="num_step" class="input-text" placeholder="请输入阶梯人数"/>&nbsp;&nbsp;
                        享受价格
                        <input type="text" name="price[]" value="0" id="enjoy_price" class="input-text" style="width: 120px;"" />
                        <img src="<?php echo G_GLOBAL_STYLE.'/global/image/asc.png'?>" onclick="addLadder(this)" title="添加价格阶梯">
                    </div>
                <?php }?>
			</td>
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" name="fare" value="<?php if(isset($act_cookie['fare'])){echo $act_cookie['fare'];}else{echo 0;}?>" id="fare" class="input-text" />
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" name="give_integral" value="<?php if(isset($act_cookie['give_integral'])){echo $act_cookie['give_integral'];}else{echo -1;}?>" id="give_integral" class="input-text" />
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" name="share_integral" value="<?php if(isset($act_cookie['share_integral'])){echo $act_cookie['share_integral'];}else{echo -1;}?>" id="share_integral" class="input-text" />
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" name="integral" value="<?php if(isset($act_cookie['integral'])){echo $act_cookie['integral'];}else{echo -1;}?>" id="integral" class="input-text" />元
		...
		<tr>
			...
			<td colspan="2">
				<input type="text" name="consult" id="consult" class="input-text wid300" placeholder="请输入手机号、座机号（区号-座机号)" value="<?php echo $act_cookie['consult']?>"/>
			</td>
		</tr>
        ...
        <tr>
			...
			<td colspan="2">
				<img id="show-poster" src="<?php if(isset($act_cookie['home_poster']) && $act_cookie['home_poster'] !='photo/goods.jpg'){echo G_UPLOAD_PATH.'/'.$act_cookie['home_poster'];}else{echo G_UPLOAD_PATH.'/photo/goods.jpg';} ?>" style="border:1px solid #eee; padding:1px; width:50px; height:50px;">
				<input type="text" id="imagehome" name="home_poster" value="<?php if(isset($act_cookie['home_poster'])){echo $act_cookie['home_poster'];}else{echo 'photo/goods.jpg';} ?>" class="input-text wid300">
				<input type="button" class="button"
             onClick="GetUploadify('<?php echo WEB_PATH; ?>','uploadify','海报图上传','image','poster',1,500000,'imagehome','call_back_poster')" value="上传图片"/>
			</td>
      	</tr>
		<tr>
			<td align="right" style="width:120px"><font color="red">*</font>活动详情海报：</td>
			<td colspan="2">
				<img id="show-img" src="<?php if(isset($act_cookie['poster']) && $act_cookie['poster'] !='photo/goods.jpg'){echo G_UPLOAD_PATH.'/'.$act_cookie['poster'];}else{echo G_UPLOAD_PATH.'/photo/goods.jpg';} ?>" style="border:1px solid #eee; padding:1px; width:50px; height:50px;">
				<input type="text" id="imagetext" name="poster" value="<?php if(isset($act_cookie['poster'])){echo $act_cookie['poster'];}else{echo 'photo/goods.jpg';} ?>" class="input-text wid300">
				<input type="button" class="button"
		...
		<tr>
			...
			<td colspan="2">
                <?php if(isset($act_cookie['notice']) && is_array($act_cookie['notice'])){?>
                    <?php foreach ($act_cookie['notice'] as $k2 => $v2){?>
                        <div class="act-notice">
                            <!--<span class="bullets">1.</span>-->
                            <input type="text" name="notice[]" class="input-text wid300" placeholder="活动须知内容" value="<?php echo $v2?>"/>
                            <?php if($k2 == 0){?>
                                <img src="<?php echo G_GLOBAL_STYLE.'/global/image/asc.png'?>" class="add-charge" onclick="copythis(this)" title="添加活动须知">
                            <?php }else{?>
                                <img src="<?php echo G_GLOBAL_STYLE.'/global/image/desc.png'?>" class="add-charge" onclick="copythis(this)" title="删除活动须知">
                            <?php }?>
                        </div>
                    <?php }?>
                <?php }else{?>
                    <div class="act-notice">
                        <!--<span class="bullets">1.</span>-->
                        <input type="text" name="notice[]" class="input-text wid300" placeholder="活动须知内容" />
                        <img src="<?php echo G_GLOBAL_STYLE.'/global/image/asc.png'?>" class="add-charge" onclick="copythis(this)" title="添加活动须知">
                    </div>
                <?php }?>
			</td>
		...
		<tr>
			...
			<td colspan="2"><textarea name="act_desc" class="wid400" onKeyUp="gbcount(this,250,'description');" style="height:60px"><?php if(isset($act_cookie['act_desc']))echo $act_cookie['act_desc']?></textarea><br /> <span>还能输入<b id="description">250</b>个字符</span>
			</td>
		</tr>
		<tr>
        	<td height="300" style="width:120px"  align="right"><font color="red">*</font>活动详情：</td>
			<td colspan="2"><script name="content" id="myeditor" type="text/plain"><?php if(isset($act_cookie['content'])) echo htmlspecialchars_decode($act_cookie['content'])?></script>
            	<style>
				.content_attr {
		...
		<tr>
			...
			<td colspan="2" id="filter">
				<?php foreach ($attribute as $val){?>
                    <?php if(isset($act_cookie['attr']) && is_array($act_cookie['attr'])){?>
                        <?php foreach ($act_cookie['attr'] as $ko => $vo){?>
                            <?php if($val['id'] == $ko){?>
                                ...
                                <input type="hidden" name="attr_id[]" value="<?php echo $val['id']?>">
		...
		<tr>
			...
			<td colspan="2">
				<input  type="text" id="share_title"  name="share_title" class="input-text wid400 bg" value="<?php if(isset($act_cookie['share_title']))echo $act_cookie['share_title']?>">
		...
		<tr>
			...
			<td colspan="2"><textarea name="share_desc" class="wid400" onKeyUp="gbcount(this,250,'textdescription');" style="height:60px"><?php if(isset($act_cookie['share_desc']))echo $act_cookie['share_desc']?></textarea><br /> <span>还能输入<b id="textdescription">250</b>个字符</span>
			</td>
		</tr>
		<tr>
			<td align="right" style="width:120px"><font color="red">*</font>分享图片：</td>
			<td colspan="2">
				<img id="share-img" src="<?php if(isset($act_cookie['share_icon']) && $act_cookie['share_icon'] !='photo/goods.jpg'){echo G_UPLOAD_PATH.'/'.$act_cookie['share_icon'];}else{echo G_UPLOAD_PATH.'/photo/goods.jpg';} ?>" style="border:1px solid #eee; padding:1px; width:50px; height:50px;">
				<input type="text" id="sharetext" name="share_icon" value="<?php if(isset($act_cookie['share_icon'])){echo $act_cookie['share_icon'];}else{echo 'photo/goods.jpg';} ?>" class="input-text wid300">
				<input type="button" class="button" onClick="GetUploadify('<?php echo WEB_PATH; ?>','uploadify','分享图片上传','image','share',1,500000,'sharetext','call_back1')" value="上传图片"/>
			</td>
		</tr>
		<!--<tr>
...
function set_title_bold(){
	...
	$(function () {
		$('#integral').change(function () {
			var intergral = parseFloat($(this).val());
