...
function pay_go_yongjin($uid=null,$dingdancode=null){
    ...
    $yq = explode(':',$config['sanji_yongjin']);
    $yqs = explode('@',$yesyaoqing['yaoqing1']);
//	var_dump($yesyaoqing);exit;
	...
	foreach($gorecode as $val){
        ...
        foreach ($yqs as $k=>$v ) {
			$s = $k+1;
			$t = explode('|',$v);
			$y_money = $val['moneycount'] * $yq[$k];
			if($v!='')$db->Query("INSERT INTO `@#_member_recodes`(`uid`,`tuid`,`jb`,`type`,`content`,`shopid`,`money`,`ygmoney`,`time`)VALUES('$uid','$t[1]','$s','1','$content','$val[shopid]','$y_money','$val[moneycount]','$time' )");           
        }
