$(function() {
    ...
    var b = function() {
        ...
        var j = function(q, t, r, yjg) {
            ...
            var s = function(w) {
                if (w.code == 1) {
                    ...
                    if (w.code == 0) {
                        q.parent().prev().html('总共购买：<em class="arial">' + r + '</em>人次/<em class="orange arial">￥' + r*yjg + "</em>")
                    }
                }
            };
        ...
        $("a[name=delLink]", a).each(function(q) {
            ...
            function() {			 
                var r = $(this);
                var t = r.attr("cartkey"); //要删除的购物车key
                var s = function() {
                    var u = function(v) {					 
                        if (v.code == 0) {
        ...
        //编辑：
        $('.z-edit').click(function(){
            var goods_id=$(this).attr('gid');
            var num=$(this).siblings('.totalPrice').find('.totalNum b').text();
			var cartkey=$(this).attr('cartkey');
            var goodsClass = new Array();
            $(this).siblings('.z-promo').find('em').each(function () {
                goodsClass.push($(this).text());
            })
            //alert(typeof goodsClass)
            //console.log(typeof goodsClass);
            $.getJSON(Gobal.Webpath+'/mobile/ajax/goodsSelect/'+goods_id+'/'+4+'/'+cartkey,function(data){
            $("body").append(data);
            
            $('#nums').text(num);
            $('.goodsClass-list li').each(function(){
                var cu=$(this);
                for (var i in goodsClass){
                    if(cu.text()===goodsClass[i]){
                        //alert(1)
                        cu.trigger('click');
                    }
                }
