...
class user extends admin {
	...
	public function reg(){
		...
		if(isset($_POST['submit-1'])){
			...
			$ip=_get_ip();
            $qx = serialize($_POST['qx']);
			$gl = $_POST['gl'];
			//echo "INSERT INTO `@#_admin` (`mid`, `username`, `userpass`, `useremail`, `addtime`, `logintime`, `loginip`,`qx`) VALUES ('$pmid', '$username', '$password', '$_POST[email]','$addtime','0','$ip','$qx')";
			
			$this->db->Query("INSERT INTO `@#_admin` (`mid`, `username`, `userpass`, `useremail`, `addtime`, `logintime`, `loginip`,`qx`,`gl`) VALUES ('$pmid', '$username', '$password', '$_POST[email]','$addtime','0','$ip','$qx','$gl')");
           // var_dump($_POST['qx']);exit;
...
//        echo $this->tpl('admin','user.list');	
	...
	public function edit(){
		...
		if(isset($_POST['submit-1'])){			
            ...
            $qx = serialize($_POST['qx']);
			$gl = $_POST['gl'];
			$ok = $this->db->Query("UPDATE `@#_admin` SET `userpass`='$password',`gl`='$gl',`qx`='$qx' WHERE (`uid`='$uid')");
	...
	public function login(){
		...
		if(isset($_POST['ajax'])){			
			...
			if(!$message['error']){				
				...
				_setcookie("AID",_encrypt($info['uid'],'ENCODE'),6*60*60);

				_setcookie("ASHELL",_encrypt(md5($info['username'].$info['userpass'])),6*60*60);
