...
function bindPrefecture() {
    ...
    for (var e = 0; e < Menus.length; e++)
        b += "\x3cli\x3e",
        b += '\x3ca href\x3d"' + Menus[e].url + '"\x3e',
        b += '\x3cspan\x3e\x3cimg src\x3d"' + imageURL + Menus[e].icon + '" height\x3d"43px"\x3e\x3c/span\x3e',
        b += "\x3cb\x3e" + Menus[e].name + "\x3c/b\x3e",
        b += "\x3c/a\x3e",
        b += "\x3c/li\x3e";
...
function bindAccumulativePeople(b) {
    ...
    for (i = 0; i < e.length; i++)
        html = "",
        html += '\x3cspan class\x3d"yNumList"\x3e',
        html += '\x3cul style\x3d"margin-top: -180px"\x3e',
        html += '\x3cli t\x3d"9"\x3e9\x3c/li\x3e',
        html += '\x3cli t\x3d"8"\x3e8\x3c/li\x3e',
        html += '\x3cli t\x3d"7"\x3e7\x3c/li\x3e',
        html += '\x3cli t\x3d"6"\x3e6\x3c/li\x3e',
        html += '\x3cli t\x3d"5"\x3e5\x3c/li\x3e',
        html += '\x3cli t\x3d"4"\x3e4\x3c/li\x3e',
        html += '\x3cli t\x3d"3"\x3e3\x3c/li\x3e',
        html += '\x3cli t\x3d"2"\x3e2\x3c/li\x3e',
        html += '\x3cli t\x3d"1"\x3e1\x3c/li\x3e',
        html += '\x3cli t\x3d"0"\x3e0\x3c/li\x3e',
        html += "\x3c/ul\x3e",
        html += "\x3c/span\x3e",
        $(".allNums").append(html);
    var f = [];
    ...
    if ($(".yNumList").length < f.length)
        for (b = f.length - $(".yNumList").length,
        i = 0; i < b; i++)
            $($(".yNumList")[0]).clone(!0).insertAfter($($(".yNumList")[$(".yNumList").length - 1]));
...
function bindIndexPublishList(b) {
    ...
    if (4 > f.length)
        for (b = b.publish_array,
        d = 0,
        k = b.length; d < k; d++)
            c = "",
            a = b[d],
            h = goodsBaseUrl + a.showImages.split(",")[0] + goodsImg,
            f = publish_detail_result_url + "-" + a.gid + "-" + a.period + ".html",
            g = $.parseJSON(a.userInfo),
            l = g.nickname,
            6 < l.length && (l = l.substring(0, 6) + "..."),
            c += '\x3cli class\x3d"c_no_border"\x3e',
            c += '\x3cdiv class\x3d"c_newest_publish_text"\x3e',
            c += '\x3cdiv style\x3d"height:40px;  overflow: hidden;line-height: 20px;"\x3e\x3ca href\x3d"' + f + '"\x3e' + a.title + "\x3c/a\x3e\x3c/div\x3e",
            c += '\x3cp class\x3d"c_publish_time"\x3e\u83b7\u5f97\u8005\uff1a\x3ca href\x3d"' + person_records_win_url + "-" + g.mid + '.html" class\x3d"c_index_a"\x3e' + l + "\x3c/a\x3e\x3c/p\x3e",
            c += "\x3c/div\x3e",
            c += '\x3cdiv class\x3d"c_newest_publish_img"\x3e',
            c += '\x3ca href\x3d"' + f + '"\x3e',
            c += '\x3cimg src\x3d"' + h + '" class\x3d"img-responsive publish" style\x3d"width:100%;/"   alt\x3d"' + a.title + '"\x3e',
            c += "\x3c/a\x3e\x3c/div\x3e",
            c += "\x3c/li\x3e",
            e.append(c);
    lazyload("publish")
}
...
function bindResult(b, e, f) {
    b = b.publishDetail;
    "" == b || null  == b ? $("#time_" + e + "_" + f).html('\x3cp class\x3d"c_publish_time c_calculation"\x3e\u6b63\u5728\u8ba1\u7b97 ...\x3c/p\x3e') : (b = $.parseJSON(b.userInfo),
    b = '\u83b7\u5f97\u8005\uff1a\x3ca  href\x3d"' + person_records_win_url + "-" + b.mid + '.html"\x3e' + b.nickname + "\x3c/a\x3e",
    $("#time_" + e + "_" + f).html(b))
}
...
function actionTimer(b, e, f) {
    var g = window.setInterval(function() {
        ...
        var d = d.toString()
          ...
          , c = c.toString()
          , h = e + "_" + f;
        36E5 <= b ? (document.getElementById("hh_0_" + h).innerHTML = d[0],
        document.getElementById("hh_1_" + h).innerHTML = d[1],
        document.getElementById("mm_0_" + h).innerHTML = k[0],
        document.getElementById("mm_1_" + h).innerHTML = k[1],
        document.getElementById("ss_0_" + h).innerHTML = c[0],
        document.getElementById("ss_1_" + h).innerHTML = c[1]) : 36E5 > b && 0 <= b ? (document.getElementById("hh_0_" + h).innerHTML = k[0],
        document.getElementById("hh_1_" + h).innerHTML = k[1],
        document.getElementById("mm_0_" + h).innerHTML = c[0],
        document.getElementById("mm_1_" + h).innerHTML = c[1],
        document.getElementById("ss_0_" + h).innerHTML = a[1],
        document.getElementById("ss_1_" + h).innerHTML = a[2]) : 0 > b && (window.clearInterval(g),
        getResult(e, f));
        b -= 10
    }, 10)
}
...
function b_fixed() {
    $(window).scroll(function() {
        $(window).scrollTop() >= $(".b_index_fixed1").offset().top ? ($(".b_float").hide(),
        $(".b_index_fixed").css({
            position: "fixed",
            top: "0",
            left: "0",
