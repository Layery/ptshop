...
//        echo 'aaaa';
		...
		if(isset($_POST['submit'])){			
            ...
            if($decode>0){
				$decode1 = 'A|'.$decode;//$code;
				$b = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `uid` = '$decode' LIMIT 1");
				if($b['yaoqing']>0){
					$decode1.='@B|'.$b['yaoqing'];
					$c = $this->db->GetOne("SELECT * FROM `@#_member` WHERE `uid` = '".$b['yaoqing']."' LIMIT 1");
					if($c['yaoqing']){
						$decode1.='@C|'.$c['yaoqing'];
					}
				}		
			...
			$sql="INSERT INTO `@#_member` (password,img,emailcode,mobilecode,reg_key,yaoqing,yaoqing1,time)VALUES('$userpassword','photo/member.jpg','-1','-1','$name','$decode','$decode1','$time')";
