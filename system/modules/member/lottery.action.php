...
class lottery extends SystemAction {
	...
	public function submit() {
		...
		} else if ( $this->userinfo['score'] <= 99 ) {
		...
		if ( $p == -1 ) {
			...
			$round = $this->round($p);
			$left = $this->userinfo['score'] -100;

			$this->db->Query("UPDATE `@#_member` SET score = score - 100,`money` = `money` + $money where (`uid` = '".$this->userinfo['uid']."')");
