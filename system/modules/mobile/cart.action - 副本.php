	...
	public function activity_pay(){
		...
		if($payment <= 0){
			...
			if($order){
				...
				$user_integral = $member['score']+$integral-$use_integral;
				$acive_balance = $member['money']-$last_balance;
			...
			if($order){
				...
				_setcookie('SignInfo','',''); //清除cookie
				_messagemobile ( "活动已失效,请重新查找活动",WEB_PATH.'/mobile/mobile/activitylists');
			}
		}
        //exit;
	}
