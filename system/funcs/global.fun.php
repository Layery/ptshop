...
function _message($string=null,$defurl=null,$time=2,$config=null){
	...
	if(defined("G_IN_ADMIN")){
		...
		if(empty($config)){
			...
			$config = array("titlebg"=>"#D80000","title"=>"#fff");
	...
	$time=intval($time);if($time<2){$time=1;}
 ...
 * @param $touser 要发送的openid
 * @param $template_id 微信模板id：需要申请模板消息
 * @param $url 模板消息的跳转链接
 * @param $data 模板内容，请按照微信开发手册填写
 * @return mixed 返回json格式的信息，请参照手机的响应参数
 */
/*function sendTemplateMsg($access_token,$post_data){
...
function downloadExcel($strTable,$filename)
{
    ...
    return '<html><meta http-equiv="Content-Type" content="text/html; charset=utf-8" />'.$strTable.'</html>';
}

/**无限极分类修改排除自己和所有的子类id
 * @param $arr 所有的分类id
 * @param $id 当前分类id
 * @return array 返回当前分类下所有的子类
 */
function getChildid($arr,$id){
    static $ids=array();
    foreach($arr as $v){
        if($v['parentid']==$id){
            $ids[]=$v['c_id'];
            getChildid($arr,$v['c_id']);
        }
    }
    return $ids;
}